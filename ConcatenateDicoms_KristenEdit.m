...
for i = 1:num_scans
    ...
    img_no = [];
    if i < 10
        match = dir(fullfile(dataPath, strcat('Cine_FLASH_flc_0', num2str(i), '*')));
    else
        match = dir(fullfile(dataPath, strcat('Cine_FLASH_flc_', num2str(i), '*')));
    end
    ...
    currentPath = fullfile(match(1).folder, match(1).name);  % Use the first match
    cd(currentPath);

    temp = dir("*.dcm");

    num_dicoms_per_folder(i) = size(temp,1);
    
    for t = 1:num_dicoms_per_folder(i)
 
        if t<10
            currentdicom = fullfile(currentPath,strcat('Image (000',num2str(t),').dcm'));
        elseif t>=100
            currentdicom = fullfile(currentPath,strcat('Image (0',num2str(t),').dcm'));
        else
            currentdicom = fullfile(currentPath,strcat('Image (00',num2str(t),').dcm'));
        end
        cur = dicominfo(currentdicom);
        pos = [pos; cur.SliceLocation];
    ...
    if length(img_no) > num_timeframes(i)
        if img_no(1) ~= img_no(num_timeframes(i)+1)
            fprintf('order of images not recognised');
            return;
        end
    end
...
if nnz(num_timeframes-num_timeframes(1)) ~= 0
    fprintf('timeframes not equal');
    return;
end

num_timeframes = num_timeframes(1);
...
for t = 1:num_timeframes
    ...
    for i = 1:num_scans
        ...
        if i <10
            currentPath = dir(strcat('Cine_FLASH_flc_0',num2str(i),'*'));
        else
            currentPath = dir(strcat('Cine_FLASH_flc_',num2str(i),'*'));
        end
        currentPath = fullfile(currentPath.folder,currentPath.name);
        
        for j = 1:num_slices_per_scan(i)

            t_cur = 1 + ((j-1)*num_timeframes) + (t-1);
            
            if t_cur<10
                current = dicomread(fullfile(currentPath,strcat('Image (000',num2str(t_cur),').dcm')));
            elseif t_cur>=100
                currentdicom = fullfile(currentPath,strcat('Image (0',num2str(t),').dcm'));
            else
                current = dicomread(fullfile(currentPath,strcat('Image (00',num2str(t_cur),').dcm')));
            end

            currentVolume(:,:,1,slice_count) = current;
...
switch (img.metaData.dimension)
 ...
 case {4}
  ...
  img.MultiVolume.NumberOfFrames = size(img.pixelData,4);
 otherwise
  assert(false, 'Unsupported pixel data dimension')
end
