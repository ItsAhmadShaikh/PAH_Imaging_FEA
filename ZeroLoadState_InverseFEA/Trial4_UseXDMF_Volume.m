...
% meshOutput.faceMaterialID = ones(size(F,1),1);
...
for q=1:size(E,1)
    ...
    febio_spec.MeshData.ElementData{1}.elem{q}.a=f0(q,:);
    febio_spec.MeshData.ElementData{1}.elem{q}.d=s0(q,:);
end
    %%
...
while 1>0
    ...
    febio_spec.Mesh.Nodes{1}.node.ATTR.id=(1:size(V_current,1))'; %The node id's
    febio_spec.Mesh.Nodes{1}.node.VAL=V_current; %The nodel coordinates
    ...
    N_disp_from_original = V_FinalDeformed - V_original;
 
    % V_update_allsteps = repmat(V_current,[1 1 size(N_disp_mat,3)]) - N_disp_from_original;
    % V_update = V_update_allsteps(:,:,end);

    V_update = V_current - N_disp_from_original;
    
    % % Visualize interior point
    cFigure; hold on;
    gpatch(Fb,V_original,'w','k',0.75);
    axisGeom; camlight headlight;
    drawnow;
    ...
    cFigure; hold on;
    gpatch(Fb,V_current,'w','k',0.75);
    axisGeom; camlight headlight;
    drawnow;

    gpatch(Fb,V_update,'w','g',0.25);
...
DN_magnitude=sqrt(sum(N_disp_mat(:,:,end).^2,2));

% Create basic view and store graphics handle to initiate animation
hf=cFigure; %Open figure
gtitle([febioFebFileNamePart,': Press play to animate']);
title('Meshes')
hp=gpatch(Fb,V_all(:,:,end),DN_magnitude,'k',1); %Add graphics object to animate
hp.Marker='.';
...
axis(axisLim(V_all)); %Set axis limits statically
camlight headlight;

% Set up animation features
animStruct.Time=timeVec; %The time vector
for qt=1:1:size(V_all,3) %Loop over time increments
    DN_magnitude=sqrt(sum(N_disp_mat(:,:,qt).^2,2)); %Current displacement magnitude

    %Set entries in animation structure
    animStruct.Handles{qt}=[hp hp]; %Handles of objects to animate
    animStruct.Props{qt}={'Vertices','CData'}; %Properties of objects to animate
    animStruct.Set{qt}={V_all(:,:,qt),DN_magnitude}; %Property values for to set in order to animate
end
